...
- 通过侧栏或分类标签筛选网站；搜索同时匹配名称、域名、描述和分类，多个关键词以空格分隔。
- 按 `⌘ K` / `Ctrl K` 或 `/` 聚焦搜索，按 `Esc` 清空搜索并退出输入。
- 点击星标收藏网站；从导航卡片打开的网站会进入最近访问，最多保留 12 个。
- 支持网格 / 列表视图、名称排序、中英文和现有四种全局主题，桌面侧栏可折叠为图标栏，手机上通过抽屉导航切换分类。
- 收藏、最近访问、视图和侧栏折叠偏好仅保存在当前浏览器，不会写入服务器；浏览器禁止存储时仍可在本次会话内使用。
- 卡片底部可展开浏览器 / 服务器检测详情；顶部连通性概览支持点击状态灯筛选和批量检测；绿色表示可达、红色表示无法访问、黄色表示检测中、灰色表示未知。
- 通过「添加网站」或「管理工作台」进入管理员入口。已登录用户刷新页面后仍会先看到导航页。
